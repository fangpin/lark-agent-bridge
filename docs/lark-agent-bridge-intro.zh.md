# 把本地 Coding Agent 接进飞书：lark-agent-bridge 的设计与实践
...
`lark-agent-bridge` 想解决的就是这件事：把飞书 / Lark 消息桥接到本地 Coding Agent，让你可以在聊天窗口里直接驱动本地的 Claude Code、Cursor Agent，或者其它兼容的 agent wrapper。
