...
- **代码块可复制**：任务完成后的卡片会给代码块加原生复制按钮（取决于飞书客户端是否支持卡片复制动作）
...
| `/new worktree <name> [backend]` | 从 `origin/main`（或 fallback 到 `origin/master`）创建 git worktree，新建带 backend 标识的群聊，并把新群 cwd/backend 绑定到该 worktree |
...
**消息策略**：私聊 = 不需要 @，任何消息都回；**群（含话题群）= 默认要 @bot 才回**（0.1.22 起的新默认），不 @ 时 bot 完全沉默；@全员永远不响应；云文档评论必须 @bot。云文档评论里的 @bot 会按文档维度排队：同一个文档里的多个评论会按到达顺序一个接一个处理，不同文档可以并发处理。要恢复"群里也不强制 @"的老行为：`/config` → "群里需要 @ bot" → 选"否"。
...
| `~/.lark-channel/queue.json` | 已接受聊天任务的持久队列；进程重启后会恢复 queued/running batch，除非用户显式停止 |
