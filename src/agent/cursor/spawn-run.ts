...
export function buildCursorPrompt(prompt: string): string {
  return `<bridge_system_prompt>\n${BRIDGE_SYSTEM_PROMPT}\n</bridge_system_prompt>\n\n<user_prompt>\n${prompt}\n</user_prompt>`;
}

export function spawnCursorRun(
  ...
  agentArgs.push(buildCursorPrompt(opts.prompt));
  ...
  log.info('agent', 'spawn', {
    pid: child.pid ?? null,
    command: spawnOpts.commandLabel,
  ...
  child.on('exit', (code, signal) => {
    log.info('agent', 'exit', { pid: child.pid ?? null, code, signal, runtime: 'cli' });
  });

  const stopGraceMs = opts.stopGraceMs ?? 5000;

  return {
    events: createEventStream(child, stderrChunks, () => runtimeError, spawnOpts.commandLabel),
    async stop() {
